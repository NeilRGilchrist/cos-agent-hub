...
This file is read by every agent at the start of every Cowork session in the hub directory. The hub is the cross-project layer: it owns the parking lot, the pattern catalog, and the project registry. Individual projects (under `projects/<name>/`) have their own `CLAUDE.md` files that govern their internal work.
...
When you're working *inside a project* (`projects/<name>/`), the role-based work happens there. When you're working *at the hub level* (here), you're triaging, reflecting, or surfacing patterns.

Hub operations are Claude Code-first by design. Bootstrapped projects are the portability layer: they use `AGENTS.md` as canonical content and add harness-specific wrappers under `.claude/` and `.cursor/`.
...
scripts/hub-index.py                                                          # rebuild cross-project FR index
scripts/parking.py <add|list|show|promote|merge|archive|reflect|reindex>
scripts/patterns.py <propose|list|show|accept|reject|mark-built|reindex>
```
...
- `template/.agent-team/**` — role definitions and escalation rules (project-level)
